...
function run(strategy, daySeed) {
  ...
  while (elapsed < MAX_TIME) {
    bird.diving = strategy(bird, terrain);
...
function raceProbe(playerStrategy, daySeed, raceSeed) {
  const terrain = new Terrain((daySeed ^ 0x2f6a1b3d) >>> 0);
  ...
  let clock = 0;
  while (clock < 300) {
    bird.diving = playerStrategy(bird, terrain);
    let acc = FRAME;
